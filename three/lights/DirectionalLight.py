...
class DirectionalLight(Light):
    ...
    def enableShadows(self, strength=0.5, bias=0.005, size=[1024, 1024]):
        ...
        self.shadowCamera.transform.setPosition(
            position[0], position[1], position[2])
        direction = self.getDirection()
        lookTarget = [position[0] + direction[0], position[1] +
                      direction[1], position[2] + direction[2]]
        self.shadowCamera.transform.lookAt(
            lookTarget[0], lookTarget[1], lookTarget[2])

        self.shadowCamera.uniformList.addUniform(
            Uniform("float", "shadowStrength", strength))
        self.shadowCamera.uniformList.addUniform(
            Uniform("float", "shadowBias", bias))
        self.shadowCamera.uniformList.addUniform(
            Uniform("vec3", "shadowLightDirection", self.getDirection()))

        self.shadowRenderTarget = RenderTarget.RenderTarget(size[0], size[1])
        self.shadowCamera.uniformList.addUniform(
            Uniform("sampler2D", "shadowMap", self.shadowRenderTarget.textureID))
