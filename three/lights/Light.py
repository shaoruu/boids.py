...
class Light(Object3D):
    ...
    def __init__(self, position=[0, 0, 0], color=[1, 1, 1], strength=1):
        ...
        self.uniformList.addUniform(
            Uniform("bool", self.name + ".isAmbient", 0),     indexName="isAmbient")
        self.uniformList.addUniform(
            Uniform("bool", self.name + ".isDirectional", 0), indexName="isDirectional")
        self.uniformList.addUniform(
            Uniform("bool", self.name + ".isPoint", 0),       indexName="isPoint")

        # default light color
        self.uniformList.addUniform(
            Uniform("float", self.name + ".strength", strength), indexName="strength")

        # control relative intensity of lights
        self.uniformList.addUniform(
            Uniform("vec3", self.name + ".color", color), indexName="color")

        # store position and direction in uniforms
        self.uniformList.addUniform(
            Uniform("vec3", self.name + ".position", position), indexName="position")
        self.uniformList.addUniform(
            Uniform("vec3", self.name + ".direction", [0, 0, 0]), indexName="direction")
