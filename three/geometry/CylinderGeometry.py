    ...
    def __init__(self, radiusTop=1, radiusBottom=1, radialSegments=32, height=2, heightSegments=2, circleTop=True, circleBottom=True):
        ...
        if circleTop:
            ...
            for i in range(radialSegments):
                posA = [radiusTop*cos(i*angle), height/2,
                        radiusTop*sin(i*angle)]
                posB = [radiusTop*cos((i+1)*angle), height/2,
                        radiusTop*sin((i+1)*angle)]
        ...
        if circleBottom:
            ...
            normal = [0, -1, 0]
            for i in range(radialSegments):
                posA = [radiusBottom*cos(i*angle), -
                        height/2, radiusBottom*sin(i*angle)]
                posB = [radiusBottom*cos((i+1)*angle), -
                        height/2, radiusBottom*sin((i+1)*angle)]
                vertexPositionData.extend([posCenter, posA, posB])
