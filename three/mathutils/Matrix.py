...
class Matrix(object):
    ...
    def __init__(self):
        self.matrix = MatrixFactory.makeIdentity().astype(
            float)  # set type, otherwise set values cast to ints
    ...
    def translateAxisDistance(self, axis=[1, 0, 0], distance=0, type=GLOBAL):
        if type == Matrix.LOCAL:
            self.matrix = self.matrix @ MatrixFactory.makeTranslationAxisDistance(
                axis, distance)
        if type == Matrix.GLOBAL:
            self.matrix = MatrixFactory.makeTranslationAxisDistance(
                axis, distance) @ self.matrix

    def rotateAxisAngle(self, axis=[1, 0, 0], angle=0, type=GLOBAL):
        if type == Matrix.LOCAL:
            self.matrix = self.matrix @ MatrixFactory.makeRotationAxisAngle(
                axis, angle)
        if type == Matrix.GLOBAL:
            self.matrix = MatrixFactory.makeRotationAxisAngle(
                axis, angle) @ self.matrix
    ...
    def lookAt(self, x, y, z):
        self.matrix = MatrixFactory.makeLookAt(
            self.getPosition(), [x, y, z], [0, 1, 0])
