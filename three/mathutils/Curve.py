...
class Curve(object):
    ...
    def calculateArcLengths(self):
        ...
        for index in range(1, self.arcLengthDivisions):
            arcLength = np.linalg.norm(
                arcLengthPoints[index] - arcLengthPoints[index-1])
    ...
    def getTangentAt(self, t):
        h = 0.00001
        tangent = np.divide(np.subtract(
            self.getPointAt(t+h), self.getPointAt(t)), h)
    ...
    def getFrames(self):
        ...
        # if tangents[0] != c*[1,1,1] else [1,1,-1]
        arbitraryVector = [1, 1, 1]
