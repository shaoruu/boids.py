...
class ParticleEngine(Mesh):
    ...
    def __init__(self, style="sphere",
        ...
        self.particleCount = self.particlesPerSecond * \
            min(self.particleDeathAge, self.emitterDeathAge)
        ...
        self.particleMaterial = ParticleMaterial(
            texture=particleTexture, additiveBlending=additiveBlending)
    ...
    def initializeParticle(self, particle):
        ...
        if self.style == "box":
            particle.position = RandomUtils.randomBoxVec3(
                self.positionBase, self.positionSpread)
            particle.velocity = RandomUtils.randomBoxVec3(
                self.velocityBase, self.velocitySpread)
        ...
        particle.color = RandomUtils.randomBoxVec3(
            self.colorBase, self.colorSpread)
        particle.opacity = RandomUtils.randomFloat(
            self.opacityBase, self.opacitySpread)
