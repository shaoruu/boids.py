...
class node():
    ...
    # array of children
    chidren = [posXposYposZ, posXposYnegZ, posXnegYposZ, posXnegYnegZ,
               negXposYposZ, negXposYnegZ, negXnegYposZ, negXnegYnegZ]
    ...
    def get_array_of_children(self):
        ...
        """
        children = [self.posXposYposZ, self.posXposYnegZ, self.posXnegYposZ, self.posXposYnegZ,
                    self.negXposYposZ, self.negXposYnegZ, self.negXnegYposZ, self.negXnegYnegZ]
        return children
            ...
            if coord[0] <= self.Xcenter:
                ...
                if coord[1] <= self.Ycenter:
                    ...
                    if coord[2] <= self.Zcenter:
                        ...
                        self.negXnegYnegZ = node(
                            self.negXnegYnegZ, Xupperlimit, Yupperlimit, Zupperlimit, Xlowerlimit, Ylowerlimit, Zlowerlimit)
          ...
          (-1,-1)-----(-1,0)-----(-1,1)
            ...
            for level in range(self.maxiter):
                for node in list_list[level]:
                    ...
                    table = ((corner0[0] > node.Xcenter), (corner0[1]
                                                           > node.Ycenter), (corner0[2] > node.Zcenter))
                    if not False in table:
                        list_list[level+1].append(node.posXposYposZ)
                    table = ((corner1[0] > node.Xcenter), (corner1[1]
                                                           > node.Ycenter), (corner1[2] < node.Zcenter))
                    if not False in table:
                        list_list[level+1].append(node.posXposYnegZ)
                    table = ((corner2[0] > node.Xcenter), (corner2[1]
                                                           < node.Ycenter), (corner2[2] > node.Zcenter))
                    if not False in table:
                        list_list[level+1].append(node.posXnegYposZ)
                    table = ((corner3[0] > node.Xcenter), (corner3[1]
                                                           < node.Ycenter), (corner3[2] < node.Zcenter))
                    if not False in table:
                        list_list[level+1].append(node.posXnegYnegZ)
                    table = ((corner4[0] < node.Xcenter), (corner4[1]
                                                           > node.Ycenter), (corner4[2] > node.Zcenter))
                    if not False in table:
                        list_list[level+1].append(node.negXposYposZ)
                    table = ((corner5[0] < node.Xcenter), (corner5[1]
                                                           > node.Ycenter), (corner5[2] < node.Zcenter))
                    if not False in table:
                        list_list[level+1].append(node.negXposYnegZ)
                    table = ((corner6[0] < node.Xcenter), (corner6[1]
                                                           < node.Ycenter), (corner6[2] > node.Zcenter))
                    if not False in table:
                        list_list[level+1].append(node.negXnegYposZ)
                    table = ((corner7[0] < node.Xcenter), (corner7[1]
                                                           < node.Ycenter), (corner7[2] < node.Zcenter))
                    if not False in table:
