...
class OctNode(object):
    ...
    def __init__(self, position, size, depth, data):
        ...
        self.lower = (position[0] - half, position[1] -
                      half, position[2] - half)
        self.upper = (position[0] + half, position[1] +
                      half, position[2] + half)
...
class Octree(object):
    ...
    def __insertNode(self, root, size, parent, position, objData):
        ...
        # else: are we not at our position, but not at a leaf node either
            ...
            root.branches[branch] = self.__insertNode(
                root.branches[branch], newSize, root, position, objData)
        ...
        # else, is this node a leaf node with objects already in it?
            ...
            if (
                ...
                for ob in objList:
                    ...
                    root.branches[branch] = self.__insertNode(
                        root.branches[branch], newSize, root, pos, ob)
        return root
...
if __name__ == "__main__":
    ...
    for tree_params in test_trees:
        ...
        if myTree.limit_nodes:
            print("Tree Leaves contain a maximum of",
                  myTree.limit, "objects each.")
        else:
